...
import os
import locale
from ctypes import CDLL, c_uint8, c_int32, c_uint32, c_double, byref, create_string_buffer, \
    POINTER, pointer, cast, c_char, c_char_p
...
class ME_SENSOR(enum.IntEnum):
    ...
    SENSOR_ILR3800 = 72  # optoNCDT ILR
...
class MEDAQLib:
    ...
    """
    current_path = current_dir = Path(__file__).parent
    if os.name == 'nt':
        medaq_lib = WinDLL(str(current_path / Path("MEDAQLib.dll")))
        medaq_lib_cdecl = CDLL(str(current_path / Path("MEDAQLib.dll")))
    else:
        medaq_lib = CDLL((str(os.getcwd() + "/libMEDAQLib.so")))
        medaq_lib_cdecl = CDLL((str(os.getcwd() + "/libMEDAQLib.so")))
    ...
    def CreateSensorInstance(Sensor):
        ...
        CreateSensorInstanceOrg.argtypes = [c_int32, ]
    ...
    def CreateSensorInstByName(sensorName):
        ...
        CreatSensorInstanceByNameOrg.argtypes = [c_char_p, ]
    ...
    def SetParameterDWORD_PTR(self, paramName, paramValue):
        ...
        :return: none
        """
        self._last_error = self._set_parameter_dword_ptr_org(self.iSensor, bytes(paramName, "utf8"), paramValue)
    ...
    def SetParameters(self, parameter_list):
        ...
        :return: none
        """
        self._last_error = self._set_parameters_org(self.iSensor, bytes(parameter_list, "utf8"))
    ...
    def GetParameterString(self, paramName, max_len=0, sensor_encoding = 'cp1252'):
        ...
        if len.value == 0:
            # call GetParameterString with 0 pointer to "ask" for size of buffer needed
            self._get_parameter_string_org(self.iSensor, bytes(paramName, "utf8"), c_char_p(), byref(len))
        param_value = create_string_buffer(len.value)
    ...
    def GetParameters(self, max_len=0, sensor_encoding = 'cp1252'):
        ...
        if len.value == 0:
            # call GetParameterString with 0 pointer to "ask" for size of buffer needed
            self._last_error = self._get_parameters_org(self.iSensor, c_char_p(), byref(len))
        param_value = create_string_buffer(len.value)
    ...
    def Poll(self, maxValues):
        ...
        self._last_error = self._poll_org(self.iSensor, cast(iValue, POINTER(c_int32)), cast(dValue, POINTER(c_double)),
                                          maxValues)
    ...
    def _transferdata_helper(self, maxValues, timeStampEnable):
        ...
        if maxValues >= 0:
            ...
            if timeStampEnable is False:
                self._last_error = self._transfer_data_org(self.iSensor, cast(raw_data_buffer, POINTER(c_int32)),
                                                           cast(scaled_data_buffer, POINTER(c_double)),
                                                           maxValues,
                                                           byref(read))
            else:
                self._last_error = self._transfer_data_ts_org(self.iSensor, cast(raw_data_buffer, POINTER(c_int32)),
                                                              cast(scaled_data_buffer, POINTER(c_double)),
                                                              maxValues,
    ...
    def init_sensor_functions(self):
        ...
        self._set_parameter_dword_ptr_org.argtypes = [c_uint32, c_char_p, POINTER(c_uint32)]
        ...
        self._set_parameter_binary_org.argtypes = [c_uint32, c_char_p, POINTER(c_uint8), c_uint32]
        ...
        self._transfer_data_org.argtypes = [c_uint32, POINTER(c_int32), POINTER(c_double), c_int32, POINTER(c_int32)]
        self._transfer_data_org.restype = c_int32

        # TransferDataTS
        self._transfer_data_ts_org = MEDAQLib.medaq_lib.TransferDataTS
        self._transfer_data_ts_org.argtypes = [c_uint32, POINTER(c_int32), POINTER(c_double), c_int32, POINTER(c_int32),
                                               POINTER(c_double)]
        self._transfer_data_ts_org.restype = c_int32

        # EnableLogging
        self._enable_logging_org = MEDAQLib.medaq_lib.EnableLogging
        self._enable_logging_org.argtypes = [c_uint32, c_int32, c_int32, c_int32, c_char_p, c_int32, c_int32, c_int32]
        self._enable_logging_org.restype = c_int32

        # LogtoFile
        self._log_to_file_org = MEDAQLib.medaq_lib_cdecl.LogToFile
        self._log_to_file_org.argtypes = [c_uint32, c_int32, c_char_p, c_char_p, POINTER(c_char)]
        ...
        self._open_sensor_if2004_usb_org.argtypes = [c_uint32, c_int32, c_char_p, c_char_p, c_int32]
